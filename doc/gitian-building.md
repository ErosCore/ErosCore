...
*Setup instructions for a gitian build of EROS Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the EROS
...
to the EROS GitHub release page.
...
- [Building EROS Core](#building-eros-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/eros-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/eros-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/eros-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/eros-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/eros-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/eros-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/Eros/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@erosblockchain.com) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/eros-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/eros-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/eros-osx-*-build.assert
```
