...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "eros" is a composite category enabling all EROS-related debug output
            if (ptrCategory->count(std::string("eros"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\EROS
// Windows >= Vista: C:\Users\Username\AppData\Roaming\EROS
// Mac: ~/Library/Application Support/EROS
// Unix: ~/.eros
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EROS";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "eros.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty eros.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override eros.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "erosd.pid"));
