...
/** Fees smaller than this (in uers) are considered zero fee (for relaying, mining and transaction creation)
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("%s : zERS spend tx %s already in block %d", __func__, tx.GetHash().GetHex(), nHeightTx),
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    if (!UpdateZERSSupplyDisconnect(block, pindex))
        return error("%s: Failed to calculate new zERS supply", __func__);
...
{
    util::ThreadRename("eros-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (!UpdateZERSSupplyConnect(block, pindex, fJustCheck)) {
    ...
    if (Params().NetworkID() == CBaseChainParams::MAIN && pindex->nHeight == consensus.height_last_ZC_WrappedSerials + 1
        ...
        RecalculateERSSupply(consensus.vUpgrades[Consensus::UPGRADE_ZC].nActivationHeight, false);
    }
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        ...
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nErosBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nErosBadBlockBits)) {
...
int ActiveProtocol()
{
    // SPORK_14 is used for 70919 (v1.1.0)
    if (sporkManager.IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
            return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 was used for 70918 (v1.0), commented out now.
