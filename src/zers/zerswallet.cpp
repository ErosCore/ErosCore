...
CzERSWallet::CzERSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zers seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZERSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZERSSeed_deprecated()) {
                    LogPrintf("%s: Updated zERS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zers seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zers wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzERSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZERSCount(nCountLastUsed);
    else if (!walletdb.ReadZERSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzERSWallet::Lock()
{
    seedMaster = 0;
}

void CzERSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzERSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZERS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzERSWallet::LoadMintPoolFromDB()
{
...
void CzERSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzERSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzERSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zersTracker->HasPubcoinHash(pMint.first)) {
...
bool CzERSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZERS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zersTracker which also adds to database
    pwalletMain->zersTracker->Add(dMint, true);
...
void CzERSWallet::SeedToZERS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzERSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzERSWallet::GenerateDeterministicZERS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzERSWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzERSWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzERSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
