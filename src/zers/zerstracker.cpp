...
#include "zerstracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
#include "wallet/walletdb.h"
#include "zers/zerswallet.h"


CzERSTracker::CzERSTracker(std::string strWalletFile)
{
...
CzERSTracker::~CzERSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzERSTracker::Init()
{
...
bool CzERSTracker::Archive(CMintMeta& meta)
{
...
bool CzERSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzERSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzERSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzERSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzERSTracker::GetSerialHashes()
{
...
CAmount CzERSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzERSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzERSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzERSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzERSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzERSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzERSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzERSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzERSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzERSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzERSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzERSWallet* zERSWallet)
{
    bool iszERSWalletInitialized = (NULL != zERSWallet);
    CMintMeta meta;
    ...
    if (! iszERSWalletInitialized)
        zERSWallet = new CzERSWallet(strWalletFile);
    meta.isSeedCorrect = zERSWallet->CheckSeed(dMint);
    if (! iszERSWalletInitialized)
        delete zERSWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzERSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzERSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzERSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzERSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzERSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzERSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzERSWallet* zERSWallet = new CzERSWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zERSWallet);
        }
        delete zERSWallet;
        LogPrint("zero", "%s: added %d dzers from DB\n", __func__, listDeterministicDB.size());
    }
