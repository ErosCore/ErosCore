...
public:
    ...
    bool WriteZERSSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZERSSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZERSSeed_deprecated(uint256& seed);
    bool EraseZERSSeed();
    bool EraseZERSSeed_deprecated();

    bool WriteZERSCount(const uint32_t& nCount);
    bool ReadZERSCount(uint32_t& nCount);
