...
#include "zers/zersmodule.h"
#include "zers/zerswallet.h"
#include "zers/zerstracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_EROS = 61000, // inherited version..
...
// Possible states for zERS send
enum ZerocoinSpendStatus {
    ZERS_SPEND_OKAY = 0,                            // No error
    ZERS_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZERS_WALLET_LOCKED = 2,                         // Wallet was locked
    ZERS_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZERS_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZERS_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZERS_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZERS_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZERS_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZERS_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZERS_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZERS_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZERS_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZERS_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZERS_SPENT_USED_ZERS = 14,                      // Coin has already been spend
    ZERS_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZERS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    /// Get 10000 ERS output and keys which can be used for the Masternode
    ...
    void doZErsRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZERS);
    ...
    bool CreateZERSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zERS wallet
    CzERSWallet* zwalletMain{nullptr};
    std::unique_ptr<CzERSTracker> zersTracker{nullptr};
    void setZWallet(CzERSWallet* zwallet);
    CzERSWallet* getZWallet();
    ...
    // zERS reset
    boost::signals2::signal<void()> NotifyzERSReset;
