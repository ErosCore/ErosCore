...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::ERS);
    ...
    if (settings.contains("nAnonymizeErosAmount") || reset)
        SoftSetArg("-anonymizeerosamount", settings.value("nAnonymizeErosAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in eros.cpp during shotdown to also remove the window positions
