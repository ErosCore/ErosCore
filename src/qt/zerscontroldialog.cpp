...
#include "ui_zerscontroldialog.h"
...
std::set<std::string> ZErsControlDialog::setSelectedMints;
std::set<CMintMeta> ZErsControlDialog::setMints;

bool CZErsControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZErsControlDialog::COLUMN_DENOMINATION || column == ZErsControlDialog::COLUMN_VERSION || column == ZErsControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZErsControlDialog::ZErsControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZErsControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
    ...
    // Title
    ui->labelTitle->setText(tr("Select zERS Denominations to Spend"));
    ui->labelTitle->setProperty("cssClass", "text-title-dialog");


    // Label Style
    ui->labelZErs->setProperty("cssClass", "text-main-purple");
    ui->labelZErs_int->setProperty("cssClass", "text-main-purple");
...
ZErsControlDialog::~ZErsControlDialog()
{
    delete ui;
}

void ZErsControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZErsControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZErsControlWidgetItem* itemDenom(new CZErsControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZErsControlWidgetItem *itemMint = new CZErsControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to spend zERS.";
            else if (!mint.isSeedCorrect)
                strReason = "The zERS seed used to mint this zERS is not the same as currently hold in the wallet";
            else
...
void ZErsControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZErsControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZErs_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    //privacyDialog->setZErsControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZErsControlDialog::GetSelectedMints()
{
...
void ZErsControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
