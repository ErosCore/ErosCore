...
#include "qt/eros/topbar.h"
#include "qt/eros/forms/ui_topbar.h"
#include <QPixmap>
#include "qt/eros/lockunlock.h"
#include "qt/eros/qtutils.h"
#include "qt/eros/receivedialog.h"
...
TopBar::TopBar(EROSGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    std::initializer_list<QWidget*> lblTitles = {ui->labelTitle1, ui->labelTitleAvailablezErs, ui->labelTitle3, ui->labelTitle4, ui->labelTitlePendingzErs, ui->labelTitleImmaturezErs};
    ...
    setCssProperty({ui->labelAmountTopErs, ui->labelAmountTopzErs}, "amount-small-topbar");
    setCssProperty({ui->labelAmountErs, ui->labelAvailablezErs}, "amount-topbar");
    setCssProperty({ui->labelPendingErs, ui->labelPendingzErs, ui->labelImmatureErs, ui->labelImmaturezErs}, "amount-small-topbar");

    ui->labelTitleAvailablezErs->setVisible(false);
    ui->labelAmountTopzErs->setVisible(false);
    ui->labelAvailablezErs->setVisible(false);
    ui->labelTitlePendingzErs->setVisible(false);
    ui->labelPendingzErs->setVisible(false);
    ui->labelTitleImmaturezErs->setVisible(false);
    ui->labelImmaturezErs->setVisible(false);
...
void TopBar::loadWalletModel() {
    ...
    // update the display unit, to not use the default ("EROS")
    updateDisplayUnit();
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    // ERS Total
    CAmount ersAvailableBalance = balance;
    // zERS Balance
    CAmount matureZerocoinBalance = zerocoinBalance - unconfirmedZerocoinBalance - immatureZerocoinBalance;

    // Set
    QString totalErs = GUIUtil::formatBalance(ersAvailableBalance, nDisplayUnit);
    QString totalzErs = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);

    // ERS
    // Top
    ui->labelAmountTopErs->setText(totalErs);
    // Expanded
    ui->labelAmountErs->setText(totalErs);
    ui->labelPendingErs->setText(GUIUtil::formatBalance(unconfirmedBalance, nDisplayUnit));
    ui->labelImmatureErs->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));

    // Update display state and/or values for zERS balances as necessary
    bool fHaveZerocoins = zerocoinBalance > 0;

    // Set visibility of zERS label titles/values
    ui->typeSpacerTop->setVisible(fHaveZerocoins);
    ui->typeSpacerExpanded->setVisible(fHaveZerocoins);
    ui->labelAmountTopzErs->setVisible(fHaveZerocoins);
    ui->zerocoinBalances->setVisible(fHaveZerocoins);

    // Top
    ui->labelAmountTopzErs->setText(totalzErs);
    // Expanded
    ui->labelAvailablezErs->setText(totalzErs);
    ui->labelPendingzErs->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));
    ui->labelImmaturezErs->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
