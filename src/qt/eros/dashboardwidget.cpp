...
#include "qt/eros/dashboardwidget.h"
#include "qt/eros/forms/ui_dashboardwidget.h"
#include "qt/eros/sendconfirmdialog.h"
#include "qt/eros/txrow.h"
...
DashboardWidget::DashboardWidget(EROSGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of ERS staked."));
    setCssSubtitleScreen(ui->labelMessage);
    setCssProperty(ui->labelSquareErs, "square-chart-ers");
    setCssProperty(ui->labelSquarezErs, "square-chart-zers");
    setCssProperty(ui->labelErs, "text-chart-ers");
    setCssProperty(ui->labelZers, "text-chart-zers");
    ui->labelZers->setVisible(false);
    ui->labelSquarezErs->setVisible(false);
    ...
    ui->labelAmountZers->setText("0 zERS");
    ui->labelAmountZers->setVisible(false);
    ui->labelAmountErs->setText("0 ERS");
    setCssProperty(ui->labelAmountErs, "text-stake-ers-disable");
    setCssProperty(ui->labelAmountZers, "text-stake-zers-disable");
    ...
    ui->btnHowTo->setText(tr("How to get ERS"));
...
void DashboardWidget::loadWalletModel(){
    ...
    // update the display unit, to not use the default ("ERS")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy() {
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isErs = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZERS;
...
bool DashboardWidget::loadChartData(bool withMonthNames) {
    ...
    chartData->amountsByCache = getAmountBy(); // pair ERS, zERS
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            ers = (pair.first != 0) ? pair.first / 100000000 : 0;
            zers = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalErs += pair.first;
            chartData->totalZers += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesErs.append(ers);
        chartData->valueszErs.append(zers);

        int max = std::max(ers, zers);
...
void DashboardWidget::onChartRefreshed() {
    ...
    // init sets
    set0 = new QBarSet("ERS");
    set1 = new QBarSet("zERS");
    set0->setColor(QColor(2,21,5));
    set1->setColor(QColor(6,33,9));
    ...
    set0->append(chartData->valuesErs);
    set1->append(chartData->valueszErs);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalErs > 0 || chartData->totalZers > 0) {
        setCssProperty(ui->labelAmountErs, "text-stake-ers");
        setCssProperty(ui->labelAmountZers, "text-stake-zers");
    } else {
        setCssProperty(ui->labelAmountErs, "text-stake-ers-disable");
        setCssProperty(ui->labelAmountZers, "text-stake-zers-disable");
    }
    forceUpdateStyle({ui->labelAmountErs, ui->labelAmountZers});
    ui->labelAmountErs->setText(GUIUtil::formatBalance(chartData->totalErs, nDisplayUnit));
    ui->labelAmountZers->setText(GUIUtil::formatBalance(chartData->totalZers, nDisplayUnit, true));
