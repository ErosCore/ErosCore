...
#include "qt/eros/forms/ui_send.h"
#include "qt/eros/addnewcontactdialog.h"
#include "qt/eros/qtutils.h"
#include "qt/eros/sendchangeaddressdialog.h"
#include "qt/eros/optionbutton.h"
#include "qt/eros/sendconfirmdialog.h"
#include "qt/eros/myaddressrow.h"
#include "qt/eros/guitransactionsutils.h"
...
SendWidget::SendWidget(EROSGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    ...
    btnContacts(new QPushButton())
{
    ...
    /* Button Group */
    ui->pushLeft->setText("ERS");
    setCssProperty(ui->pushLeft, "btn-check-left");
    ui->pushLeft->setChecked(true);
    ui->pushRight->setText("zERS");
    setCssProperty(ui->pushRight, "btn-check-right");
    ui->pushRight->setVisible(false);
    ui->pushLeft->setVisible(false);

    /* Subtitle */
    ui->labelSubtitle1->setText(tr("You can transfer public coins (ERS)"));
    ...
    /* Address */
    ui->labelSubtitleAddress->setText(tr("Enter EROS address or contact label"));
    ...
    ui->labelAmountSend->setText("0.00 ERS");
    ...
    setCssProperty(coinIcon, "coin-icon-ers");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onERSSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onERSSelected(false);});
...
void SendWidget::refreshAmounts() {
    ...
    bool isZers = ui->pushRight->isChecked();
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();

    ui->labelAmountSend->setText(GUIUtil::formatBalance(total, nDisplayUnit, isZers));
    ...
    if (CoinControlDialog::coinControl->HasSelected()){
        ...
        // Wallet's balance
        totalAmount = (isZers ? walletModel->getZerocoinBalance() : walletModel->getBalance()) - total;
...
void SendWidget::onSendClicked(){
    ...
    bool sendErs = ui->pushLeft->isChecked();

    // request unlock only if was locked or unlocked for mixing:
    // this way we let users unlock by walletpassphrase or by menu
    // and make many transactions while unlocking through this dialog
    // will call relock
    if(!GUIUtil::requestUnlock(walletModel, sendErs ? AskPassphraseDialog::Context::Send_ERS : AskPassphraseDialog::Context::Send_zERS, true)){
        // Unlock wallet was cancelled
        inform(tr("Cannot send, wallet locked"));
        return;
    }

    if((sendErs) ? send(recipients) : sendZers(recipients)) {
...
bool SendWidget::sendZers(QList<SendCoinsRecipient> recipients){
    if (!walletModel || !walletModel->getOptionsModel())
        return false;

    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        Q_EMIT message(tr("Spend Zerocoin"), tr("zERS is currently undergoing maintenance."), CClientUIInterface::MSG_ERROR);
        return false;
    }
    ...
    // use mints from zERS selector if applicable
    std::vector<CMintMeta> vMintsToFetch;
    std::vector<CZerocoinMint> vMintsSelected;
    if (!ZErsControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZErsControlDialog::GetSelectedMints();
    ...
    if (walletModel->sendZers(
        ...
        inform(tr("zERS transaction sent!"));
        ZErsControlDialog::setSelectedMints.clear();
        clearAll();
        return true;
    } else {
        QString body;
        if (receipt.GetStatus() == ZERS_SPEND_V1_SEC_LEVEL) {
            body = tr("Version 1 zERS require a security level of 100 to successfully spend.");
        } else {
            int nNeededSpends = receipt.GetNeededSpends(); // Number of spends we would need for this transaction
            const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zERS transaction
        ...
        Q_EMIT message("zERS transaction failed", body, CClientUIInterface::MSG_ERROR);
        return false;
    }
}
...
void SendWidget::onCoinControlClicked(){
    if(isERS){
        if (walletModel->getBalance() > 0) {
            ...
            inform(tr("You don't have any ERS to select."));
        }
    }else{
        if (walletModel->getZerocoinBalance() > 0) {
            ZErsControlDialog *zErsControl = new ZErsControlDialog(this);
            zErsControl->setModel(walletModel);
            zErsControl->exec();
            ui->btnCoinControl->setActive(!ZErsControlDialog::setSelectedMints.empty());
            zErsControl->deleteLater();
        } else {
            inform(tr("You don't have any zERS in your balance to select."));
        }
    }
}
...
void SendWidget::onERSSelected(bool _isERS){
    isERS = _isERS;
    setCssProperty(coinIcon, _isERS ? "coin-icon-ers" : "coin-icon-zers");
    refreshView();
...
void SendWidget::onContactMultiClicked(){
    if(focusedEntry) {
        ...
        CBitcoinAddress ersAdd = CBitcoinAddress(address.toStdString());
        if (walletModel->isMine(ersAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(ersAdd.Get(), dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
