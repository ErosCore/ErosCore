...
#include "qt/eros/forms/ui_addresseswidget.h"
#include "qt/eros/addresslabelrow.h"
#include "qt/eros/addnewaddressdialog.h"
...
AddressesWidget::AddressesWidget(EROSGUI* parent) :
    PWidget(parent),
    ui(new Ui::AddressesWidget)
{
    ...
    // Address
    ui->labelAddress->setText(tr("Enter EROS address"));
...
void AddressesWidget::onStoreContactClicked(){
    if (walletModel) {
        ...
        CBitcoinAddress ersAdd = CBitcoinAddress(address.toUtf8().constData());
        ...
        if (walletModel->updateAddressBookLabels(ersAdd.Get(), label.toUtf8().constData(),
                ersAdd.IsStakingAddress() ? AddressBook::AddressBookPurpose::COLD_STAKING_SEND : AddressBook::AddressBookPurpose::SEND)
                ) {
