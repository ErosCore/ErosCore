...
#include "qt/eros/forms/ui_privacywidget.h"
#include "qt/eros/qtutils.h"
#include "guiutil.h"
#include "qt/eros/denomgenerationdialog.h"
#include "qt/eros/txviewholder.h"
...
PrivacyWidget::PrivacyWidget(EROSGUI* parent) :
    PWidget(parent),
    ui(new Ui::PrivacyWidget)
{
    ...
    /* Subtitle */
    ui->labelSubtitle1->setText(tr("Minting zERS anonymizes your ERS by removing any\ntransaction history, making transactions untraceable "));
    setCssSubtitleScreen(ui->labelSubtitle1);

    ui->labelSubtitle2->setText(tr("Mint new zERS or convert back to ERS"));
    ...
    ui->lineEditAmount->setPlaceholderText("0.00 ERS ");
    ...
    ui->labelValueDenom1->setText("0x1 = 0 zERS");
    ...
    ui->labelValueDenom5->setText("0x5 = 0 zERS");
    ...
    ui->labelValueDenom10->setText("0x10 = 0 zERS");
    ...
    ui->labelValueDenom50->setText("0x50 = 0 zERS");
    ...
    ui->labelValueDenom100->setText("0x100 = 0 zERS");
    ...
    ui->labelValueDenom500->setText("0x500 = 0 zERS");
    ...
    ui->labelValueDenom1000->setText("0x1000 = 0 zERS");
    ...
    ui->labelValueDenom5000->setText("0x5000 = 0 zERS");
    setCssProperty(ui->labelValueDenom5000, "text-body2");

    ui->layoutDenom->setVisible(false);

    // List
    ui->labelListHistory->setText(tr("Last zERS Movements"));
    ...
    // Only Convert to ERS enabled.
    ui->containerViewPrivacyChecks->setVisible(false);
    onMintSelected(false);

    ui->btnTotalzERS->setTitleClassAndText("btn-title-grey", "Total 0 zERS");
    ui->btnTotalzERS->setSubTitleClassAndText("text-subtitle", "Show denominations of zERS owned.");
    ui->btnTotalzERS->setRightIconClass("ic-arrow");

    ui->btnCoinControl->setTitleClassAndText("btn-title-grey", "Coin Control");
    ui->btnCoinControl->setSubTitleClassAndText("text-subtitle", "Select ERS outputs to mint into zERS.");
    ...
    ui->btnResetZerocoin->setTitleClassAndText("btn-title-grey", "Reset Spent zERS");
    ui->btnResetZerocoin->setSubTitleClassAndText("text-subtitle", "Reset zerocoin database.");

    connect(ui->btnTotalzERS, SIGNAL(clicked()), this, SLOT(onTotalZersClicked()));
...
void PrivacyWidget::onMintSelected(bool isMint){
    QString btnText;
    if(isMint){
        btnText = tr("Mint zERS");
        ui->btnCoinControl->setVisible(true);
        ui->labelSubtitleAmount->setText(tr("Enter amount of ERS to mint into zERS"));
    }else{
        btnText = tr("Convert back to ERS");
        ui->btnCoinControl->setVisible(false);
        ui->labelSubtitleAmount->setText(tr("Enter amount of zERS to convert back into ERS"));
    }
...
void PrivacyWidget::onTotalZersClicked(){
    bool isVisible = ui->layoutDenom->isVisible();
    if(!isVisible){
        ui->layoutDenom->setVisible(true);
        ui->btnTotalzERS->setRightIconClass("btn-dropdown", true);
    }else{
        ui->layoutDenom->setVisible(false);
        ui->btnTotalzERS->setRightIconClass("ic-arrow", true);
    }
}
...
void PrivacyWidget::onSendClicked(){
    ...
    if(sporkManager.IsSporkActive(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        warn(tr("Zerocoin"), tr("zERS is currently undergoing maintenance"));
        return;
    }
    ...
    if(!GUIUtil::requestUnlock(walletModel, AskPassphraseDialog::Context::Mint_zERS, true)){
        inform(tr("You need to unlock the wallet to be able to %1 zERS").arg(isConvert ? tr("convert") : tr("mint")));
        return;
    }
...
void PrivacyWidget::mint(CAmount value){
    ...
    if(!walletModel->mintCoins(value, CoinControlDialog::coinControl, strError)){
        ...
        // Mint succeed
        inform(tr("zERS minted successfully"));
        // clear
...
void PrivacyWidget::spend(CAmount value){
    ...
    if(!walletModel->convertBackZers(
            value,
            selectedMints,
            receipt
    )){
        ...
        // Spend succeed
        inform(tr("zERS converted back to ERS"));
        // clear
        ui->lineEditAmount->clear();
    }
}
...
void PrivacyWidget::updateDenomsSupply(){
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zERS </b>";
    ...
    ui->btnTotalzERS->setTitleText(tr("Total %1").arg(GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true)));
}
