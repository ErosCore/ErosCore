...
#include "qt/eros/forms/ui_snackbar.h"
#include "qt/eros/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(EROSGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &EROSGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
