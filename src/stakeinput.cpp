...
bool CErsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CErsStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CErsStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal)
{
...
CDataStream CErsStake::GetUniqueness() const
{
    //The unique identifier for a ERS stake is the outpoint
...
CBlockIndex* CErsStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
