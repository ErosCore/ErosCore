...
bool initStakeInput(const CBlock& block, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight) {
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CLegacyZErsStake(spend));
        ...
        if (nPreviousBlockHeight < Params().Zerocoin_Block_V2_Start() ||
            ...
            return error("%s : zERS stake block: height %d outside range", __func__, (nPreviousBlockHeight+1));
        CLegacyZErsStake* zERS = dynamic_cast<CLegacyZErsStake*>(stake.get());
        if (!zERS) return error("%s : dynamic_cast of stake ptr failed", __func__);
        // The checkpoint needs to be from 200 blocks ago
        const int cpHeight = nPreviousBlockHeight - Params().Zerocoin_RequiredStakeDepth();
        const libzerocoin::CoinDenomination denom = libzerocoin::AmountToZerocoinDenomination(zERS->GetValue());
        if (ParseAccChecksum(chainActive[cpHeight]->nAccumulatorCheckpoint, denom) != zERS->GetChecksum())
        ...
        CErsStake* ersInput = new CErsStake();
        ersInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(ersInput);
    }
    return true;
}
