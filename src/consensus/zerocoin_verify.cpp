...
bool RecalculateERSSupply(int nHeightStart, bool fSkipZers)
{
    ...
    while (true) {
        ...
        if (!fSkipZers && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
            UpdateZERSSupplyConnect(block, pindex, true);
        }
