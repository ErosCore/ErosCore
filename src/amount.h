...
/** Amount in ERS (Can be negative) */
typedef int64_t CAmount;

static const CAmount COIN = 100000000;
static const CAmount CENT = 1000000;

/** No amount larger than this (in ERS) is valid in a single UTXO.
 *
 * Note that this constant is *not* the total money supply, which in EROS
 ...
 */
static const CAmount MAX_MONEY_OUT = 35000000 * COIN;
inline bool MoneyRange(const CAmount& nValue) { return (nValue >= 0 && nValue <= MAX_MONEY_OUT); }

/**
 * Fee rate in ERS per kilobyte: CAmount / kB
 */
class CFeeRate
{
private:
